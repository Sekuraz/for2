...
# https://github.com/MyNameIsArko/RL-Flax/blob/main/PPO.py

class ActorCritic(nn.Module):
    def __init__(self, *a, **kwargs):
        super(ActorCritic, self).__init__(*a, **kwargs)
...
def parse_args():
    ...
    parser.add_argument('--num-envs', type=int, default=32, help='the number of parallel environments')
...
if __name__ == '__main__':
    ...
    actor = Actor(action_n=4)  # For jit we need to declare prod outside of class
    critic = Critic()
    ...
    mazes = np.load("mazes.npy")
    paths = np.load("mazes.npy")

    observations, actions = to_observations(mazes, paths)
    ...
    obs = jnp.load("observations.npy")
    actions = jnp.load("actions.npy")
    dones = jnp.load("done.npy")
    rewards = jnp.load("rewards.npy")

    # Initialize the storage
    storage = Storage(
        obs=obs,
        actions=actions,
        logprobs=jnp.zeros_like(actions),
        dones=dones,
    ...
    try:
        ...
        orbax_checkpointer.save(os.path.abspath("model"), ckpt, save_args=save_args)
