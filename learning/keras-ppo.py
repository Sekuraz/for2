...
from helper.maze_gymnasium import MazeEnv

from data.data_classes import Openings
...
for epoch in tqdm.tqdm(range(epochs)):
    ...
    # # supervise a little more
    # if epoch % 100 == 0:
    #     X, y, _ = env.unwrapped.get_training_data(batch=128)
    #     X_test, y_test, _ = env.unwrapped.get_training_data(batch=32)
    #     actor.fit(X, y, epochs=5, batch_size=32, validation_data=(X_test, y_test))
    ...
    for t in range(steps_per_epoch):
        ...
        # print(Openings(10, np_array=observation[0][2:], position=observation[0][:2]),
        #       env.unwrapped._action_to_opening[action.numpy()[0]])
