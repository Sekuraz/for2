...
Maze.prototype.makeMaze = function() {
	...
	const mazeSize = this.nodes.length;
	...
	let moveNodes = [];
	let visited = 0;
	let position = parseInt(Math.floor(Math.random() * this.nodes.length), 10);
	...
	this.nodes[position] = replaceAt(this.nodes[position], 0, 1);
	...
	while (visited < (mazeSize - 1)) {
		...
		if (directions.length) {
			...
			this.nodes[position] = replaceAt(this.nodes[position], positionIndex[direction], 0);
			// Set new position
			position = next[direction];

			// Update next position
			this.nodes[position] = replaceAt(this.nodes[position], oppositeIndex[direction], 0);
			this.nodes[position] = replaceAt(this.nodes[position], 0, 1);
		} else {
			if (!moveNodes.length) {
				break;
...
Maze.prototype.makeMatrix = function() {
	...
	for (let i = 0; i < mazeSize; i++) {
		...
		if (stringVal(this.nodes[i], 1)) {
			...
			let hasAbove = this.nodes.hasOwnProperty(i - this.width);
			let above = hasAbove && stringVal(this.nodes[i - this.width], 4);
			let hasNext = this.nodes.hasOwnProperty(i + 1);
			let next = hasNext && stringVal(this.nodes[i + 1], 1);
