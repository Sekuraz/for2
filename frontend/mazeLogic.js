...
function initMaze() {
    ...
    document.getElementById("notice").innerHTML =
        "Maze is ready. <br />Press <i>Enter</i> to start.<br />Press <i>Esc</i> to show settings and help.</div>";
...
function importMaze(mazeDataJson) {
    ...
    optimalPath = mazeData.optimalPath;
...
function createMaze() {
    ...
    validExits = ["right", "bottom", "left", "top"];  // valid movments for all cells
    ...
    const oPath = addRoute(startAtRow, startAtCol, false);
    normalizeOptimalPath(oPath);
...
function normalizeOptimalPath(path) {
    let pos = [1, 1];
    for (const new_pos of path) {
        if (pos[1] > new_pos[1]) {
            optimalPath += "l";
        }
